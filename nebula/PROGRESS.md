...
| **Improvement-reward RL (entry 88)** | **OWNER-APPROVED; PREREGISTRATION ONLY.** D18 freezes a new policy-untouched 2,448-identity TEST complement, scale-free q-improvement reward, invalid-action masks, one-move-before-LOCK rule, TRAIN-only controls, five new 200k-step seeds and unchanged +0.0200 quality/safety gates. No Entry 88 code, artifact, training step or TEST score exists yet. |
| Test suite | Entry 87 final evidence state: **2631 passed**, 13 deselected, 2 warnings in 297.13 s. D16 production adoption baseline: 2607 passed. The **system** interpreter, not the conda env -- that env has no `torch`, and `ngspice_con.exe` is found by absolute path anyway (G69) |
| Gates G0–G2 | passed |
...
| **4aj** | **Test improvement-reward masked PPO on Entry 87's policy-untouched complement.** TRAIN is all 2,592 previously exposed identities; FINAL TEST is the other 2,448. Controls first, then five registered seeds | 0 SPICE; 1,000,000 table steps | **D18 APPROVED; PREREGISTERED, NOT IMPLEMENTED.** Commit the exact contract before environment/control code. Never inspect FINAL TEST before all policies are frozen. |
