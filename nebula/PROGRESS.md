...
| Test suite | Entry-86 final: **2606 passed**, 13 deselected, 2 warnings in 343.54 s. The same 2606 passed before reanalysis in 304.31 s; focused Entry-86 group: 26/26. The **system** interpreter, not the conda env -- that env has no `torch`, and `ngspice_con.exe` is found by absolute path anyway (G69) |
| Gates G0–G2 | passed |
