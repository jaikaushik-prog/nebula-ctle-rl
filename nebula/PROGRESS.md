...
| Test suite | **2583 passed**, 13 deselected, 290.02 s, measured 2026-09-03 after entry 83; 2 warnings (`python -m pytest tests nebula/tests -q -m "not slow"`). Entry-82 baseline: **2574 passed**, 13 deselected. Focused entry-83 suite: **74 passed**. The **system** interpreter, not the conda env — that env has no `torch`, and `ngspice_con.exe` is found by absolute path anyway (G69) |
| Gates G0–G2 | passed |
...
| **Adjacent-Cs closure probe (entry 84)** | **OWNER-APPROVED AND PRE-REGISTERED; not yet implemented or run.** Exactly three real-PMOS rows keep 7 dB and move C1 to C2 at the three remaining high-side frequency misses. |
