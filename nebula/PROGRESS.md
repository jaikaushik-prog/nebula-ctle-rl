...
| **Margin-adaptation RL (entry 87)** | **PPO IMPLEMENTED AND TESTED; NO REGISTERED TRAINING YET.** Controls-first commit `52dabac` freezes fixed at 99.31% compliance/q=0.6796/one trial. Categorical PPO and crash-safe five-seed evaluation pass the 23-test focused group; full suite 2630/2630. Next: commit implementation, then train seeds 00..04 one at a time. |
...
| **4ai** | **Train margin-adaptation RL on the frozen 512-code table.** Hidden PVT/channel, receiver-visible eye history, request-conditioned start; controls first, then five registered categorical-PPO seeds | 0 SPICE; 1,000,000 table steps | **IMPLEMENTED, NOT TRAINED.** Stage 1 controls are committed. Categorical PPO, one-seed durable artifacts, deterministic evaluation and Q1-Q8 scorer pass 23 focused tests and 2630 full tests. Commit before running seed `2026090300`. |
