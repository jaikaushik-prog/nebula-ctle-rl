...
| **Improvement-reward RL (entry 88)** | **STAGE 1 COMPLETE; NO POLICY CODE OR TEST SCORE.** Sixteen fail-first gates and 2,647 full tests pass. TRAIN fixed is 99.31%/q=0.6906; the seven-move reachable oracle is 100%/q=0.9890, leaving +0.2984 headroom. Random/hill-climb are unsafe. Zero SPICE; FINAL TEST remains `DEFINED_NOT_SCORED`. Commit controls before masked PPO. |
...
| **4aj** | **Test improvement-reward masked PPO on Entry 87's policy-untouched complement.** TRAIN is all 2,592 previously exposed identities; FINAL TEST is the other 2,448. Controls first, then five registered seeds | 0 SPICE; 1,000,000 table steps | **STAGE 1 READY TO COMMIT.** TRAIN-only reachable q gain is +0.2984 within 7 moves; FINAL TEST remains unscored. Commit environment, controls, artifact, tests and handoff before any policy code. |
