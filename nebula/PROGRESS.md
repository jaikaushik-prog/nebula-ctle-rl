...
| **Margin-adaptation RL (entry 87)** | **PPO IMPLEMENTED; ZERO TRAINING STEPS.** Controls-first commit `52dabac`; policy commit `c1e0791`. Seed 00 stopped before reset because displayed request keys lost precision. A fail-first exact-key reader repair passes 2631 full tests; commit it, then retry seed 00. Comparator remains 99.31% compliance/q=0.6796/one trial. |
...
| **4ai** | **Train margin-adaptation RL on the frozen 512-code table.** Hidden PVT/channel, receiver-visible eye history, request-conditioned start; controls first, then five registered categorical-PPO seeds | 0 SPICE; 1,000,000 table steps | **IMPLEMENTED; PRE-STEP REPAIR READY.** Seed 00 exposed lossy `:g` request keys before any step/artifact. Exact registered-key reconstruction now passes 2631 tests. Commit repair, then retry seed 00; no gate or artifact changed. |
