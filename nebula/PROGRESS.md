...
| **4ag** | **Resolve the final 3 dB row without moving a human-controlled range silently.** Existing R5C2 is below the peaking-match floor; R6C2 needs 0.263140 dB less input swing as a lower bound. The owner approved an exact 7.3 dB one-point diagnostic as D14 | exactly 1 deck, then full verification only if it passes | **PRE-REGISTERED as entry 85; not implemented or run.** Do not infer another increase if it misses (G145) |
