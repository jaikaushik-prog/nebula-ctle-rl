...
| Test suite | **2582 passed**, 13 deselected, 382.17 s, measured 2026-09-02 (entry 83 implementation); 2 warnings (`python -m pytest tests nebula/tests -q -m "not slow"`). Entry-82 baseline: **2574 passed**, 13 deselected. The **system** interpreter, not the conda env — that env has no `torch`, and `ngspice_con.exe` is found by absolute path anyway (G69) |
| Gates G0–G2 | passed |
...
| **7.0 dB diagnostic (D12, entry 83)** | **IMPLEMENTED AND TESTED; not yet run.** Exactly 16 real-PMOS invocations will test the 14 unique critical points behind the remaining 16 short-channel failures, plus TT code-0/code-7 controls. The official D11 range remains 5.933 dB. Focused 73/73; full non-slow suite 2,582 passed. |
...
| **4ae** | **Run the owner-approved 7.0 dB focused probe.** Exercise the 14 unique critical circuit/corner points covering all 16 failures, plus TT code-0/code-7 controls; score 3 dB requests, 12 dB scorability, noise and realised attenuation | 16 real-PMOS SPICE invocations | **IMPLEMENTED AND GREEN; not yet run.** Entry 83 fixes all six gates. This is a diagnostic candidate only; passing does not adopt it |
