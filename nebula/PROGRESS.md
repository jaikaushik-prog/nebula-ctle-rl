...
| **Safety-shielded RL rescue (entry 89)** | **STAGE 1 CORE IMPLEMENTED; NO POLICY OR FRESH ROW EXISTS.** Eight fail-first gates cover the DEVELOPMENT-only oracle teacher and simulator-backed best-compliant-visited shield. Next implement BC + unchanged PPO and DEVELOPMENT diagnostics. Five policies must freeze before a new 23,040-row midpoint-loss journal; FINAL is 2,430 identities scored once. |
| Test suite | Entry 89 Stage 1: **2670 passed**, 13 deselected, 2 warnings in 537.99 s; its focused shield/oracle group passes 8/8. The Entry 88 result-path repair remains covered. The **system** interpreter, not the conda env -- that env has no `torch`, and `ngspice_con.exe` is found by absolute path anyway (G69) |
| Gates G0–G2 | passed |
...
| **4ai** | **Train margin-adaptation RL on the frozen 512-code table.** Hidden PVT/channel, receiver-visible eye history, request-conditioned start; controls first, then five registered categorical-PPO seeds | 0 SPICE; 1,000,000 table steps | **DONE; Q6 FAIL.** All five seeds are healthy and Q1-Q5/Q7-Q8 pass. RL nearly always locks immediately: +0.00287 mean q versus the required +0.0200. TEST is now exposed; no tuning or rerun. Fixed lookup remains production fallback. |
| **4aj** | **Test improvement-reward masked PPO on Entry 87's policy-untouched complement.** TRAIN is all 2,592 previously exposed identities; FINAL TEST is the other 2,448. Controls first, then five registered seeds | 0 SPICE; 1,000,000 table steps | **POLICY IMPLEMENTED, NOT TRAINED.** Stage 1 is commit `6e3231e`; masked PPO and crash-safe evaluator pass 30 focused/2,661 full tests. Commit implementation, then run seed `2026090400` alone. |
