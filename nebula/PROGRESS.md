...
**Updated 2026-09-03, session 38.** 12 days to the 15 Sept deadline.
...
| **Full 7.3 dB bank verification (entry 86)** | **COMPLETE: Q1-Q7 PASS.** All 23,040 rows completed uninterrupted in 131.27 min with zero hard device failures. Coverage is **16/16 at every loss from 3-12 dB**; 3 dB scorable rows improve 7,519 -> 10,378. The adoption recommendation passes, but D11 remains unchanged until the owner decides. See `JOINT_BANK_73_RESULTS.md`. |
| **Margin-adaptation RL (entry 87)** | **ENVIRONMENT + CONTROLS COMPLETE; NO POLICY CODE OR TRAINING YET.** The fixed TRAIN-only lookup is the registered comparator: 99.31% compliance, q=0.6796, one trial. RL floors are 98.31% compliance and q>=0.6996 plus positive paired CI. Fourteen focused tests pass; control artifact used zero SPICE. |
| Test suite | D16 production adoption: **2607 passed**, 13 deselected, 2 warnings in 278.92 s; focused attenuator/joint-bank group 66/66. Clean pre-change baseline: 2606 passed in 302.26 s. The **system** interpreter, not the conda env -- that env has no `torch`, and `ngspice_con.exe` is found by absolute path anyway (G69) |
| Gates G0–G2 | passed |
...
| **Input attenuator (D11, superseded by D16)** | **7.3 dB PRODUCTION RANGE ADOPTED.** D11's 1.98x/5.93 dB minimum remains historically reproducible. Entry 86 verified D16's exact 2.3173946499684783x range across 23,040 real-PMOS bank/PVT points: zero hard failures and 16/16 coverage at every channel loss. |
| **7.0 dB diagnostic (D12-D13, entry 83)** | **Block-level SUCCESS; system-level Q4 remains a recorded MISS.** 7.0176 dB realised, TT noise 0.5357 mV_rms, and 14/14 critical points scorable at both 3 and 12 dB. It immediately recovers 13/16 requests; three miss only frequency matching. The owner clarified the intended attenuator scope in D13, so 7 dB is retained as the compression candidate while production D11 still awaits full verification. |
| **Adjacent-Cs closure probe (entry 84)** | **COMPLETE: 5/6.** Exactly three real-PMOS rows moved C1 to C2; frequency moved correctly at 3/3 and two rows closed. The last became a compression-only miss, leading to entry 85's 7.3 dB probe. |
...
| **D14** | **Approve one exact 7.3 dB top-code diagnostic at the final R6C2 row** (owner, 2026-09-03, session 37) | **COMPLETE: entry 85 passes 6/6 and focused closure reaches 16/16.** Exactly one `sf/0.95/125C`, request-12, bank-50 real-PMOS invocation ran. This authorises proposing a separately preregistered full verification only; D11 remains unchanged and the large run is still unauthorised. |
| **D15** | **Approve the full 7.3 dB bank verification** (owner, 2026-09-03, session 38) | **COMPLETE: Q1-Q7 PASS.** The 23,040-row run recommends adoption: 16/16 coverage at every loss and zero hard failures. It does not pre-authorise production adoption; no RL training was performed. |
| **D16** | **Adopt Entry 86's verified 7.3 dB attenuator range** (owner, 2026-09-03, session 39) | Production `ATTEN_MAX_X` becomes the exact measured ratio 2.3173946499684783. The old 1.98x D11 range remains available explicitly for historical reproduction. This authorises the analog default change, not unspecified RL reward weights. |
| **D17** | **Approve Entry 87's exact margin-adaptation RL contract** (owner, 2026-09-03, session 39) | Authorises the frozen 512-code table environment, D10 weights `-1/-60/+20q`, 8-trial horizon, registered train/test split, controls-first sequence, categorical PPO defaults, five 200k-step seeds and Q1-Q8. No SPICE and no post-result tuning. |
...
| **4ag** | **Resolve the final 3 dB row without moving a human-controlled range silently.** Existing R5C2 is below the peaking-match floor; R6C2 needs 0.263140 dB less input swing as a lower bound. The owner approved an exact 7.3 dB one-point diagnostic as D14 | exactly 1 deck, then full verification only if it passes | **DONE: 6/6 PASS, focused closure 16/16.** The one measured row has 0.293754 dB lower DC gain, both links scorable and 0.7367804 mV_rms noise. Next is an owner-approved, separately preregistered full-bank/45-corner verification proposal; D11 is unchanged. |
| **4ah** | **Verify the 7.3 dB candidate across the complete bank and mandated PVT grid.** Compare against entry 81 without changing the design, request grid, tolerances, load or channels | exactly 23,040 decks before internal retries | **DONE: Q1-Q7 PASS.** 23,040/23,040 rows, zero hard failures, 16/16 coverage at every loss, 131.27 min. The original false-Q1 artifact is preserved beside the corrected zero-SPICE result; owner adopted it as D16. |
| **4ai** | **Train margin-adaptation RL on the frozen 512-code table.** Hidden PVT/channel, receiver-visible eye history, request-conditioned start; controls first, then five registered categorical-PPO seeds | 0 SPICE; 1,000,000 table steps | **STAGE 1 COMPLETE; COMMIT BEFORE POLICY CODE.** Environment leak/accounting/reward gates pass 14/14. Controls: fixed 0.9931 compliance/q 0.6796/1 trial; hill 0.4722/0.4222/7.405; random 0.5090/0.3487/4.968; exhaustive-eye 0/0/512; oracle 1/1/1. Next: commit artifact SHA `4E7930...E09501D`, then implement categorical PPO red-first. |
