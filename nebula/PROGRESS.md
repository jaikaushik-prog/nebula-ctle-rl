...
| Test suite | **2574 passed**, 13 deselected, 435.56 s, measured 2026-09-02 (session 36); 2 warnings (`python -m pytest tests nebula/tests -q -m "not slow"`). Point-2 baseline: **2572 passed**, 13 deselected. The **system** interpreter, not the conda env — that env has no `torch`, and `ngspice_con.exe` is found by absolute path anyway (G69) |
| Gates G0–G2 | passed |
...
| **7.0 dB diagnostic (D12, entry 83)** | **APPROVED AND PRE-REGISTERED; not yet implemented or run.** Exactly 16 real-PMOS invocations will test the 14 unique critical points behind the remaining 16 short-channel failures, plus TT code-0/code-7 controls. The official D11 range remains 5.933 dB. |
...
| **4ae** | **Run the owner-approved 7.0 dB focused probe.** Exercise the 14 unique critical circuit/corner points covering all 16 failures, plus TT code-0/code-7 controls; score 3 dB requests, 12 dB scorability, noise and realised attenuation | 16 real-PMOS SPICE invocations | **REGISTERED, entry 83; not yet implemented or run.** This is a diagnostic candidate only. All six gates must pass before even proposing a full-bank/PVT adoption experiment |
