...
| **D14** | **Approve one exact 7.3 dB top-code diagnostic at the final R6C2 row** (owner, 2026-09-03, session 37) | Entry 85 permits exactly one `sf/0.95/125C`, request-12, bank-50 real-PMOS invocation, scored at 3 and 12 dB. This is measurement permission only. D11 and the full 23,040-row table remain unchanged and unauthorised. |
...
| **4ag** | **Resolve the final 3 dB row without moving a human-controlled range silently.** Existing R5C2 is below the peaking-match floor; R6C2 needs 0.263140 dB less input swing as a lower bound. The owner approved an exact 7.3 dB one-point diagnostic as D14 | exactly 1 deck, then full verification only if it passes | **IMPLEMENTED, NOT RUN.** Entry 85's hash-pinned one-row driver and all six fail-capable gates are green; commit before the one authorised SPICE call. Do not infer another increase if it misses (G145) |
