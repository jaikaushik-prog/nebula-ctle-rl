"""Masked categorical PPO for Entry 88's seven-action controller.

Entry 87's unmasked implementation remains untouched.  Masks are applied both
when collecting rollouts and when recomputing log probabilities during PPO
updates, so unavailable actions cannot silently re-enter the objective.
"""
...
from nebula.rl.discrete_ppo import (
    DiscretePPOConfig, DiscretePPOStats, state_dict_sha256,
)


class MaskedActorCritic(nn.Module):
    """Separate masked categorical-policy and scalar-value tanh trunks."""
    ...
    def distribution(self, obs: torch.Tensor,
        ...
        if mask.shape != logits.shape:
            raise ValueError(
                f"mask shape {tuple(mask.shape)} != logits {tuple(logits.shape)}")
        if not bool(mask.any(dim=-1).all().item()):
            raise ValueError("every observation needs at least one available action")
...
def deterministic_action(net: MaskedActorCritic, obs: np.ndarray,
                         mask: np.ndarray) -> int:
    with torch.no_grad():
        tensor = torch.as_tensor(obs, dtype=torch.float32).unsqueeze(0)
        available = torch.as_tensor(mask, dtype=torch.bool).unsqueeze(0)
        dist = net.distribution(tensor, available)
...
def train(env, cfg: DiscretePPOConfig) -> tuple[MaskedActorCritic,
    ...
    while stats.steps_completed < cfg.total_steps:
        ...
        for _ in range(cfg.rollout_steps):
            ...
            tensor = torch.as_tensor(obs, dtype=torch.float32).unsqueeze(0)
            t_mask = torch.as_tensor(mask, dtype=torch.bool).unsqueeze(0)
            with torch.no_grad():
                dist = net.distribution(tensor, t_mask)
                action = dist.sample()
        ...
        t_masks = torch.as_tensor(np.asarray(action_masks), dtype=torch.bool)
        ...
        for _ in range(cfg.epochs):
            ...
            for start in range(0, cfg.rollout_steps, batch_size):
                ...
                dist = net.distribution(t_obs[index], t_masks[index])
...
__all__ = (
    "DiscretePPOConfig", "DiscretePPOStats", "MaskedActorCritic",
