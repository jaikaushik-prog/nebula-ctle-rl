...
PDF=OUT/'Nebula_Final_Submission_12p_20260915_v2.pdf'
...
V2_ASSETS=ROOT/'nebula/report/final_submission_v2_assets'
SCRATCH=ROOT/'tmp/final-submission-report-v2'
...
def require_completed_revision():
    if not REVISION_READY:
        raise ValueError('V2 is not publishable: frozen campaign metrics are not yet integrated')
...
class FinalReport:
    ...
    def table(self,headings,rows,widths=None,size=9.5):
        ...
        padding={2:9,3:5,4:11,9:10,10:7,12:5}.get(self.n,7)
    ...
    def figure(self,name,caption,height):
        ...
        path=(V2_ASSETS if name=='desktop' else ASSETS)/(name+'.png')
...
def build(e=None, regenerate_assets=False):
    ...
    if regenerate_assets:
        raise ValueError('V2 reuses immutable v1 figures; generate new revision assets separately')
    ...
    r.table(['LAYER','WHAT RUNS / WHAT IS EXPORTED','WHAT ITS PASS MEANS'],[
     ...
     ['Fixed physical CTLE','Eligible fixed setting, physical bias/MIM and exact captured transistor deck. Automatic recovery retains rejected candidates and tries the next eligible fixed circuit.','One fixed CTLE across 315 modeled conditions. Its DFE remains ideal behavioral cancellation.'],
    ...
    r.heading('Declared physical target coverage')
    ...
    r.table(['BOOST','1.25 GHz','1.90 GHz','2.50 GHz'],[
        [f'{boost:g} dB']+[labels[grid[boost,freq*1e9]['status']] for freq in (1.25,1.9,2.5)]
        for boost in (3.,6.,9.,12.)],[71,147,147,r.width-365],size=9.2)
    r.note('Measured scope.', f'Three of twelve grid requests delivered; 6 dB / 2.5 GHz failed fresh physical verification and eight had no fixed cached candidate. The exposed 6 dB / 2.1 GHz diagnostic recovered automatically, giving four delivered requests out of thirteen. Each success uses one fixed CTLE across 315 model conditions. Coverage cost: {campaign['coverage_calls']} charged calls; {campaign['coverage_parent_s']:.3f} s summed workflow time. The full tuning rectangle remains unverified. [E11]')
    ...
    recovered=campaign['selected'];meas=campaign['design']['nominal']['meas']
    ...
    r.para(f"The completed output measures {meas['peaking_db']:.4f} dB at {meas['_f_peak_ghz']:.4f} GHz. The full parent workflow took <b>{recovered['parent_wall_s']:.3f} s and {recovered['spice_calls']} fresh SPICE calls</b>, including the rejected candidate, parsing, process startup, selection, verification, exact deck export, drawing, files and receipt. Request limits remain +/-1.5 dB and +/-0.3 octave.",size=10.3)
    ...
    r.new('10 / Product and demonstration','The recovery application connects the target, measured attempts and one completed physical result.')
    r.figure('desktop','Recovery workbench showing the completed 6 dB / 2.1 GHz physical result, with its circuit, response, specifications and run evidence. [E9,E11]',244)
    r.table(['DEMO STEP','WHAT THE REVIEWER CAN INSPECT'],[
        ['Request / recovery','Open the saved 6 dB / 2.1 GHz recovery on port 8766. Inspect the retained 288 rejection (314/315) and setting 352 acceptance (315/315).'],
        ['Design Explorer',f"Recovered setting 352: {meas['peaking_db']:.4f} dB / {meas['_f_peak_ghz']:.4f} GHz; nominal modeled eye {meas['eye_h_v']*1000:.3f} mV / {meas['eye_w_ui']:.6f} UI. Inspect CTLE, tails and physical reference."],
        ['Design PVT',f"One fixed circuit; 45 corners x seven losses = 315/315. Across all per-loss links, minimum modeled height {campaign['min_eye_h_v']*1000:.3f} mV and width {campaign['min_eye_w_ui']:.6f} UI."],
        ['Compare / checkpoint','Compare the saved AC-derived CTLE/ideal-DFE views. Open the separately labeled 9 dB integrated transistor receiver checkpoint; do not combine its verification with this recovery.'],
        ['Channel / Run files','Inspect the target, both attempt records, exact deck, schematic and measured specs. Uploaded Touchstone profiles do not inherit cached or fixed-PVT acceptance.'],
    ],[111,r.width-111],size=9.2)
    r.para('Launch: <b>py -3.13 -m nebula.web.recovery_server</b>; open <b>http://127.0.0.1:8766</b>. The earlier app remains separately available. Identify saved evidence explicitly. The archived demonstrated run took 166.085 s; a new request has its own receipt. Any shortened recording must identify its time compression.',size=10.1)
    r.note('Demonstration boundary.', 'The target is met within declared tolerances. The app includes later file-I/O hardening; recorded campaign timings predate that change. The recovered CTLE uses ideal behavioral DFE scoring. The separate integrated transistor checkpoint retains its own nominal analog and finite-pattern link-PVT scope.')
    ...
    r.para('Nebula connects target parsing, learned proposals, deterministic selection, fresh SPICE acceptance and exact circuit output. Automatic recovery delivered the exposed 6 dB / 2.1 GHz request after rejecting its first candidate. The completed finite grid distinguishes delivered results, physical rejection and missing cached candidates; the matched benchmark bills both successful and unsuccessful work. All 37 workflows total 4,110 charged invocations.')
    r.note('Remaining engineering work.', 'Expand the physically verified target domain; close analog PVT, DFE-active retuning, clock/control generation and extracted layout. The recovered fixed CTLE and separate calibrated transistor receiver remain distinct evidence layers.')
    r.heading('Source-to-claim guide')
    r.para('Audit paths below are relative to <b>nebula/product_audits/</b>. Campaign files are under <b>submission_recovery_20260915/</b>. The companion source manifest gives exact paths and hashes.',size=8.8,gap=8)
    r.table(['KEY / PAGES','SOURCE ARTIFACT / CLAIM'],[
        ['E1-E2 / 1,2,5-9','entry143_dfe_calibrated_verification_20260910/ and entry144_dfe_calibrated_pvt_20260910/ - nominal analog and fixed-control transistor link PVT.'],
        ['E3 / 3,4','nebula/experiments/shielded_policy_final_results.json; nebula/rl/hybrid_designer.py - frozen policy study and proposal logic.'],
        ['E4-E5 / 4,10','entry113_attribution_20260907/; entry115_exhaustive_benchmark_20260908/ - matched cached controls, cost ledger and failed near-optimality gate.'],
        ['E6 / 3,5-7','nebula/product_demo/physical_bias_9db_1p9ghz_20260906/; entry115_physical_recovery_20260908/ - earlier fixed CTLE and recovered targets.'],
        ['E7 / 1,2,5','CLAUDEwa.md - deliverables and S1-S9. Existing SerDes infrastructure is declared; all reported circuit results are Nebula NRZ.'],
        ['E8 / 10','nebula/product_demo/rl_hybrid_9db_1p9ghz/design.json; nebula/AUTOMATION_RECEIPT_20260914.md - historical adaptive-bank receipt.'],
        ['E9 / 11','nebula/web/recovery_server.py; nebula/report/final_submission_v2_assets/desktop.png - recovery app and completed-result screenshot.'],
        ['E10 / 9','submission_analog_pvt_20260914/second_attempt/summary.json - nominal replay; stress latch initialization failed.'],
        ['E11 / 3,10,11','Campaign workflows.jsonl; coverage_012_6db_2p1ghz_rl_r0/output/ - both actual attempts, selected setting 352, exact deck and 315-condition evidence.'],
        ['E12 / 10','Campaign protocol.json, summary.json, workflows.jsonl and final_review.json - frozen schedule, all-workflow timing and independent deep check.'],
    ],[72,r.width-72],size=8.2)
    r.para('<b>Methods.</b> [R1] <link href="https://skywater-pdk.readthedocs.io/en/main/">SkyWater SKY130 official documentation</link>. [R2] <link href="https://ngspice.sourceforge.io/docs.html">ngspice documentation</link>. [R3] Schulman et al., <i>Proximal Policy Optimization Algorithms</i>, 2017, <link href="https://arxiv.org/abs/1707.06347">arXiv:1707.06347</link>. Measurements are E1-E12.',size=8.6,gap=7)
    r.para('Build: <b>py -3.13 -m nebula.report.final_submission_v2</b>; check: <b>py -3.13 -m nebula.report.check_final_submission_v2</b>. Sources/review JSON files bind evidence and page checks. The completed v1 PDF and all earlier reports are preserved. Nine legacy JSON newline mismatches remain a documented clean-clone rebuild limit (HANDOFF G197).',size=8.6,gap=0)
    r.save()
    sources=[ROOT/'nebula/product_audits/submission_analog_pvt_20260914/second_attempt/summary.json',DETAILED_PDF.with_name(DETAILED_PDF.stem+'_sources.json'),Path(__file__),ROOT/'nebula/report/check_final_submission_v2.py',ROOT/'nebula/report/submission_story.py',ROOT/'nebula/report/competition_submission.py',NOMINAL_SUMMARY,PVT_SUMMARY,NOMINAL_NETLIST,FINAL,RECEIPT_SOURCE,PHYS/'summary.json',PHYS/'evidence_sha256.json',WIN_BENCH/'summary.json',WIN_BENCH/'sha256.json',WIN_RECOVERY/'summary.json',WIN_RECOVERY/'sha256.json',ROOT/'nebula/POST_REVIEW_RESULTS.md',ROOT/'nebula/DFE_CALIBRATED_RESULTS.md',ROOT/'CLAUDEwa.md',ROOT/'nebula/rl/hybrid_designer.py',ROOT/'nebula/web/static/app.js',ROOT/'nebula/web/static/design_plots.js',ROOT/'nebula/web/design_visuals.py',ROOT/'nebula/web/hardware_checkpoint.py',ROOT/'nebula/web/hardware_visuals.py',ROOT/'nebula/experiments/evidence_archive.py',ROOT/'nebula/AUTOMATION_RECEIPT_20260914.md',ROOT/'nebula/results/submission_receipt_20260914/receipt.json']
