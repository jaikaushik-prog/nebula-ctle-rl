...
PDF=OUT/'Nebula_Final_Submission_12p_20260915_v3.pdf'
...
SCRATCH=ROOT/'tmp/final-submission-report-v3'
...
class FinalReport:
    ...
    def figure(self,name,caption,height):
        ...
        path=(V3_ASSETS if name in ('desktop','generated_aperture') else ASSETS)/(name+'.png')
...
def build(e=None, regenerate_assets=False):
    ...
    r.table(['SPECIFICATION','REQUIRED TARGET','EVIDENCE / ASSESSMENT'],[
     ...
     ['S2 / Topology','CTLE, variable Rs/Cs,<br/>one-tap DFE','Generated CTLE + transistor DFE export; physical variable Rs/Cs in the independent reference.'],
     ['S3 / Response','3-12 dB; peak 1.25-2.5 GHz','Physical evidence at 4/12 grid targets; Cs refinement adds 6 dB / 2.5 GHz. Full rectangle incomplete.'],
     ['S4 / Linearity','HD3 below -30 dBc,<br/>100 MHz input','Reference clocked HD3 about -54.92 dBc; 100 mV differential peak.'],
     ['S5 / Noise','Below 1.5 mVrms,<br/>10 MHz-5 GHz','Reference held-clock noise at most 0.65405 mVrms; periodic noise unverified.'],
     ['S6 / Power','Below 15 mW','Reference link-PVT maximum 12.5241 mW; external generators excluded.'],
     ['S7 / Area','Below 0.05 mm2, 130 nm','Reference geometry 0.014715766 mm2; not routed layout area.'],
     ['S8 / Eye','Above 100 mV / 0.4 UI','Generated nominal: 296.981 mV / 0.720 UI. Independent reference PVT minimum: 113.244 mV / 0.635 UI.'],
     ['S9 / PVT','5 process x 3 supplies x<br/>3 temperatures','Generated CTLE: 45 x 7 modeled conditions. Reference transistor link: 45/45. Generated DFE PVT open.'],
    ...
    r.new('04 / Circuit narrative','The fixed export explains the CTLE core; the independent reference adds voltage controls.')
    ...
    r.figure('desktop','Reviewed 1440x1000 workbench: saved setting 352, actual 288-to-352 recovery trace, selected CTLE and modeled DFE labels. Screenshot run 94b55321 preserves the earlier accepted circuit; the 164.766106 s live receipt is run 4608cf1c. [E9,E11]',224)
    r.table(['WORKFLOW','WHAT THE REVIEWER CAN INSPECT'],[
        ...
        ['Modeled DFE / new receiver','The 315-condition eye gate uses ideal behavioral DFE outside design.cir. New accepted outputs separately include receiver/receiver.cir and hashed structural metadata; no automatic nominal/full verification claim.'],
        ...
        ['Run files / audit','Inspect exact decks, measurements and attempts. Nested receiver downloads require recorded hashes; changed or unrecorded paths are refused.'],
    ...
    r.para('Launch: <b>py -3.13 -m nebula.web.recovery_server</b>; <b>http://127.0.0.1:8766</b>. Desktop QA passes 1440x1000 and 1280x900. Identify saved evidence and time compression in any recording; each new physical request has its own receipt.',size=9.7)
    ...
    r.table(['KEY / PAGES','SOURCE ARTIFACT / CLAIM'],[
        ...
        ['E3-E5 / 3,4,10','nebula/experiments/shielded_policy_final_results.json; entry113_attribution_20260907/; entry115_exhaustive_benchmark_20260908/ - frozen policy, matched controls and cost ledger.'],
        ...
        ['E9 / 11','nebula/web/recovery_server.py; nebula/report/final_submission_v3_assets/desktop.png - recovery app and completed-result screenshot.'],
        ['E10 / 9','submission_analog_pvt_20260914/second_attempt/summary.json - nominal replay; stress latch initialization failed.'],
        ['E11 / 3,10,11','Campaign workflows.jsonl; nebula/product_demo/submission_runs_20260915_v2/4608cf1c525f4e59b2d5226059b0ce5d/ - actual live recovery, 274 charges and 164.766106 s.'],
        ...
        ['E13 / 2,3,12','continuous_cs_coverage_20260915/summary.json; nebula/CONTINUOUS_CS_COVERAGE_REVIEW_20260915.json - three-factor gate and retained raw manifest.'],
        ['E14 / 1,2,3,8,12','generated_receiver_6db_2p1ghz_20260915/result.json and independent_review.json - selected CTLE + transistor DFE, nominal signal pass and signed-domain limit.'],
    ],[72,r.width-72],size=8.2)
    r.para('<b>Methods.</b> [R1] <link href="https://skywater-pdk.readthedocs.io/en/main/">SkyWater SKY130 official documentation</link>. [R2] <link href="https://ngspice.sourceforge.io/docs.html">ngspice documentation</link>. [R3] Schulman et al., <i>Proximal Policy Optimization Algorithms</i>, 2017, <link href="https://arxiv.org/abs/1707.06347">arXiv:1707.06347</link>. Measurements are E1-E14.',size=8.6,gap=7)
    r.para('Build: <b>py -3.13 -m nebula.report.final_submission_v3</b>; check: <b>py -3.13 -m nebula.report.check_final_submission_v3</b>. Sources/review JSON files bind evidence and page checks. The completed v1/v2 PDFs and earlier reports are preserved. Nine legacy JSON newline mismatches remain a documented clean-clone rebuild limit (HANDOFF G197).',size=8.6,gap=0)
    r.save()
