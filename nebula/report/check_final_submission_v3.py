...
import shutil
import subprocess
from nebula.report.final_submission_v3 import PDF,MANIFEST,REVIEW,SCRATCH,ROOT,OUT,PRESERVED,RAW_PROOF,NOMINAL_ROOT,PVT_ROOT,sha,verify_sources,verify_frozen_baseline,require_completed_revision,load_campaign_evidence,load_revision_evidence
...
def main():
    ...
    assert manifest['campaign_summary']==campaign['summary']
    assert manifest['revision_evidence']==load_revision_evidence()
    ...
    review=dict(report_sha256=sha(PDF),page_count=12,source_hash_checks='PASS',raw_archive_proof='PASS (Entry143/144 completed verification inherited from preserved proof; current exact dependencies checked)',previous_reports='PASS',text_bounds='PASS',outline='PASS',figure_sequence=figures,blank_acroform_fields='PASS (11)',renderer='Poppler pdftoppm 120 dpi',pages=stats,visual_review='PENDING')
