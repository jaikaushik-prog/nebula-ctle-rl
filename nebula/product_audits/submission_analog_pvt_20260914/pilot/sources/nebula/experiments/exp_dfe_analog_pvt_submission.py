...
def call_allowed(elapsed,calls):
    return bool(calls<MAX_CALLS and elapsed+spice_capture.MAX_SECONDS<=WALL_BUDGET_SECONDS)
...
def run(out):
    ...
    with hold('dfe_slicer'):
        ...
        def evaluate(corner,args):
            ...
            if not call_allowed(time.perf_counter()-started,calls):
