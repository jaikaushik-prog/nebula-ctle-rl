...
def candidate_vid_max(candidate):
    """One existing definition: preserve intrinsic sweep after input attenuation."""
    from nebula.experiments.adaptive_screen import _attenuation_run_args
    search = candidate['search']
    return _attenuation_run_args(search['atten_code'], search['atten_max_x'])['vid_max']


def measurement_deck(source, tone, amplitude, *, swing, vid_max=None):
    ...
    if swing:
        if vid_max is None or not math.isfinite(vid_max) or vid_max <= 0:
            raise ValueError('swing requires an explicit attenuation-adjusted vid_max')
        deck += S._SWING_BLOCK.format(device=P.NFET, vid_max=vid_max, vid_step=.004)
                ...
                for name, tone, amplitude, swing in [('s4', S.HD3_TONE_HZ, S.HD3_VIN_DIFF_PK_V, False),
                    ...
                    try:
                        cir = measurement_deck(deck, tone, amplitude, swing=swing,
                                               vid_max=candidate_vid_max(candidate) if swing else None)
                        calls += 1
                        row['spice_calls'] += 1
