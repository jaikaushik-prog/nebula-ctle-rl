...
# Bounded spoken values used by the target grid. Units remain mandatory; this
# is not a general English-number parser. Signed values still reach S3 validation.
_WORD_PEAKING = {"three": 3.0, "six": 6.0, "nine": 9.0, "twelve": 12.0}
_WORD_DB = re.compile(
    r"(?<!\w)(?P<sign>[-+]\s*|(?:minus|negative|plus|positive)\s+)?"
    r"(?P<word>three|six|nine|twelve)\s*(?:d\s*b|decibels?)\b", re.I)
...
def parse_offline(text: str) -> ParsedRequest:
    ...
    word_db_m = _WORD_DB.search(text)
    f_m = _FREQ.search(text)

    if word_db_m and (db_m is None or word_db_m.start() < db_m.start()):
        peaking = _WORD_PEAKING[word_db_m.group('word').lower()]
        sign = (word_db_m.group('sign') or '').strip().lower()
        if sign in ('-', 'minus', 'negative'):
            peaking = -peaking
    elif db_m:
