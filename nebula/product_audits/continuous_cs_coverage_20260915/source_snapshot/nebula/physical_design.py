...
def select_fixed_setting(table, request, nominal_setting, losses, forced_setting=None):
    ...
    if forced_setting is not None:
        forced_setting = int(forced_setting)
        if forced_setting not in choices:
            raise ValueError('forced setting is not eligible for every characterised condition')
        return forced_setting, choices, checks
    return (nominal_setting if nominal_setting in eligible else choices[0]), choices, checks


def propose(peaking_db, f_peak_hz, loss_db, forced_setting=None):
...
def is_verified(design):
    ...
    records = v.get('per_condition') or []
    losses = v.get('channel_losses_db') or []
    expected = {(V.A._label(c), loss) for c in all_corners() for loss in losses}
    membership = {(r.get('corner'), r.get('channel_loss_db')) for r in records}
    signature = evidence.get('circuit_signature')
    setting = (design.get('search') or {}).get('setting')
    return bool(losses and len(losses) == len(set(losses)) and signature and setting is not None
                and len(records) == len(expected) and membership == expected
                and all(r.get('compliant') and r.get('circuit_signature') == signature
                        and r.get('setting') == setting for r in records)
                and v.get('n_points') == len(expected) and v.get('n_pass') == len(expected)
                and v.get('n_failed') == 0 and v.get('n_channel_losses') == len(losses)
                and v.get('mode') == MODE and v.get('all_points_pass')
                ...
                and fixed.get('setting') == setting and fixed.get('dfe_control_all_pass'))
...
def _snapshots(out, extra_sources=()):
    paths = [Path(__file__), Path(P.__file__), Path(V.__file__), Path(S.__file__),
             ...
             S.SPICE_DIR / '.spiceinit', *extra_sources]
    folder = out / 'inputs'
    folder.mkdir()
    hashes = {}
...
def _record_call_progress(out, calls, stage, invocation_state, elapsed):
    pending = out / 'call_progress.json.tmp'
    P.write_json(pending, dict(spice_calls=calls, charged_invocations=calls,
        stage=stage, invocation_state=invocation_state, wall_s=elapsed,
        count_scope='Charged immediately before invocation; not confirmation that ngspice launched or completed.'))
    # Retry only transient file replacement, never a charge or simulator call.
    for attempt in range(7):
        try:
            pending.replace(out / 'call_progress.json')
            break
...
def run(peaking_db, f_peak_hz, *, evidence_dir, channel_loss_db=None, progress=None,
        forced_setting=None, prepared_proposal=None):
    ...
    with hold('physical_product', here=out):
        try:
            _snapshots(out, (ROOT / 'nebula/physical_recovery.py',) if prepared_proposal is not None else ())
            update('Prepared fixed candidate (legacy evidence only)' if prepared_proposal is not None
                   else 'RL proposal and fixed-setting pre-screen (legacy evidence only)', 10)
            if prepared_proposal is None:
                ...
                if forced_setting is not None or channel_loss_db is not None:
                    raise ValueError('prepared proposal cannot combine with setting/loss overrides')
                if prepared_proposal['request'] != {'peaking_db': float(peaking_db), 'f_peak_hz': float(f_peak_hz)}:
                    raise ValueError('prepared proposal request differs')
                search, old = deepcopy(prepared_proposal['search']), deepcopy(prepared_proposal['legacy'])
                if (search['setting'] not in search['fixed_eligible_settings']
                        or search['channel_losses_db'] != list(LOSSES_DB)):
                    raise ValueError('prepared proposal must retain fixed eligibility and all channel losses')
                P.write_json(out / 'prepared_proposal.json', prepared_proposal)
