...
def test_static_batch_holds_clock_and_records_controls():
    ...
    assert 'alter VrcC 0.333' in P.static_batch(design,source,0,[(.7,.185),(.7,.185)])
    assert 'Vdfclk df_clk 0 0.6' in deck
    assert '.nodeset ' in deck and '.ic ' not in deck
    ref=json.loads(P.B.original_file(P.B.LOADED,P.B.baseline_folder(1)+'/result.json').read_text())
    assert f"v(df_mp)={ref['dc_nodes_v']['df_mp']:.16g}" in deck
