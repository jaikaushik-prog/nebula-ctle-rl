"""Entry 140: Gear-2 recovery of aborted clocked distortion traces."""
import argparse
import json
...
ROOT=P.ROOT
PRIOR=ROOT/'nebula/product_audits/entry139_dfe_clocked_linearity_20260910'
SOURCES=tuple(sorted(set(P.SOURCES+('nebula/DFE_LINEARITY_GEAR_PLAN.md',
    'nebula/experiments/exp_dfe_linearity_gear.py','nebula/tests/test_dfe_linearity_gear.py'))))


def deck(step,reference):
    if step not in L.STEPS: raise ValueError('unregistered Gear time step')
    text=I.deck('tone',0,step,-1,reference)
    if 'method=' in text.lower(): raise ValueError('existing integration method')
    return text.replace('.control','.options method=gear maxord=2\n.control',1)
...
def prerequisites():
    ...
    if summary['entry']!=139 or summary['spice_calls']!=2 or summary['clocked_ctle_hd3_model_pass']:
        raise ValueError('expected retained failed Entry 139 pair')
...
def schedule(evaluate):
    result=P.schedule(evaluate)
    result.update(entry=140,integration_method='gear',maximum_order=2)
    return result
...
def run(out):
    with hold('dfe_slicer'):
        ...
        calls=0
        result={'entry':140,'clocked_ctle_hd3_model_pass':False,'full_receiver_verified':False}
        try:
            models=pdk_hashes()
            ...
            W.write_json(out/'config.json',{'entry':140,'max_calls':2,'timeout_s':spice_capture.MAX_SECONDS,
            ...
            def evaluate(step):
                ...
                try:
                    text=deck(step,reference);calls+=1
                ...
                row['max_step_s']=step;W.write_json(folder/'result.json',row)
