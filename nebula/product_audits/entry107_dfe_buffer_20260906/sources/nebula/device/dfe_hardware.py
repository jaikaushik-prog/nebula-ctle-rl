"""Entry 106/107 MOS decision/hold prototypes. NOT a complete transistor DFE.
...
def dut_lines(width_um, *, buffered=False):
    ...
    if buffered:
        ...
        for raw, middle, output in (('x', 'bx1', 'bx'), ('y', 'by1', 'by')):
            for stage, inp, out in ((1, raw, middle), (2, middle, output)):
                mos(f'buf_{raw}{stage}_n', f'{out} {inp} 0 0')
                mos(f'buf_{raw}{stage}_p', f'{out} {inp} vdd vdd', True)
...
def geometry(width_um, *, buffered=False):
    dut_lines(width_um, buffered=buffered)
...
def deck(width_um, corner: Corner, common_mode_v: float, bits, *, buffered=False):
    ...
    for node, sign in (('inp', 1), ('inn', -1)):
        ...
        lines.append('+ )')
    lines.extend(dut_lines(width_um, buffered=buffered))
