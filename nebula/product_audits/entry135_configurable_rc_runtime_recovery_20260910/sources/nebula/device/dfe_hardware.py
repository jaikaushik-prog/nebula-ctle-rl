"""Entry 106-108 MOS decision/hold prototypes. NOT a complete transistor DFE.
...
WIDTHS_UM = (4, 8, 16)
BUFFER_DRIVE_PAIRS_UM = ((8, 8), (8, 16), (16, 8), (16, 16))
CHAIN_WIDTHS_UM = (2, 1)
L_UM = .15
...
def dut_lines(width_um, *, buffered=False, buffer_drive_um=None, chain_width_um=None):
    if width_um not in WIDTHS_UM:
        raise ValueError('width is not in the registered 4/8/16 um set')
    if buffer_drive_um is not None and (not buffered or width_um != 4
                                       or buffer_drive_um not in BUFFER_DRIVE_PAIRS_UM):
        raise ValueError('unapproved buffer drive pair or non-4-um buffered core')
    if chain_width_um is not None and (not buffered or width_um != 4
            or buffer_drive_um is not None or chain_width_um not in CHAIN_WIDTHS_UM):
        raise ValueError('unapproved lower-load width or incompatible circuit option')
    lines = []

    def mos(name, nodes, p=False, ratio=1, width_override=None):
        w = width_um * ratio if width_override is None else width_override
        peripheral = chain_width_um is not None and name.startswith(('buf_', 'q_', 'qb_'))
        if peripheral:
            w = chain_width_um
        nf = 1 if peripheral else w / 2
        lines.append(f'X{name} {nodes} {PFET if p else NFET} '
                     f'w={w:g} l={L_UM:g} nf={nf:g}')
    ...
    if buffered:
        # Entry 107: two real CMOS inversions retain the active-low polarity.
        for raw, middle, output in (('x', 'bx1', 'bx'), ('y', 'by1', 'by')):
            for stage, inp, out in ((1, raw, middle), (2, middle, output)):
                n_width = buffer_drive_um[1] if buffer_drive_um is not None and stage == 2 else None
                p_width = buffer_drive_um[0] if buffer_drive_um is not None and stage == 1 else None
                mos(f'buf_{raw}{stage}_n', f'{out} {inp} 0 0', width_override=n_width)
                mos(f'buf_{raw}{stage}_p', f'{out} {inp} vdd vdd', True, width_override=p_width)
    # Cross-coupled static NANDs. Legacy connections remain the default.
    set_n, reset_n = ('bx', 'by') if buffered else ('x', 'y')
    for out, active_low, other in (('q', set_n, 'qb'), ('qb', reset_n, 'q')):
...
def geometry(width_um, *, buffered=False, buffer_drive_um=None, chain_width_um=None):
    dut_lines(width_um, buffered=buffered, buffer_drive_um=buffer_drive_um, chain_width_um=chain_width_um)
    result = {'mos_count': 19, 'input_total_width_um': width_um,
            'length_um': L_UM, 'gate_geometry_mm2': 20 * width_um * L_UM / 1e6,
            'full_area_mm2': None, 'scope': 'MOS gate W*L only, not layout area'}
    if buffered:
        result.update(mos_count=27, buffer_mos_count=8,
                      gate_geometry_mm2=28 * width_um * L_UM / 1e6)
    if buffer_drive_um is not None:
        result.update(first_stage_pmos_width_um=buffer_drive_um[0],
                      second_stage_nmos_width_um=buffer_drive_um[1],
                      gate_geometry_mm2=(24 * width_um + 2 * sum(buffer_drive_um)) * L_UM / 1e6)
    if chain_width_um is not None:
        result.update(chain_width_um=chain_width_um, chain_nf=1,
                      gate_geometry_mm2=(12 * width_um + 16 * chain_width_um) * L_UM / 1e6)
    return result


def terminal_nodes(lines):
    return sorted({node for line in lines for node in line.split()[1:5]} - {'0'})


def deck(width_um, corner: Corner, common_mode_v: float, bits, *, buffered=False, buffer_drive_um=None, chain_width_um=None):
    ...
    if buffered:
        lines[0] = '* Entry 107: buffered decision and hold; external clock/input, no DFE feedback'
    if buffer_drive_um is not None:
        lines[0] = '* Entry 108: targeted buffer drive; external clock/input, no DFE feedback'
    if chain_width_um is not None:
        lines[0] = '* Entry 111: lower-load buffer/storage; external clock/input, no DFE feedback'
    for node, sign in (('inp', 1), ('inn', -1)):
        ...
        lines.append('+ )')
    devices = dut_lines(width_um, buffered=buffered, buffer_drive_um=buffer_drive_um, chain_width_um=chain_width_um)
    ...
    if chain_width_um is not None:
        lines.append('wrdata terminals.txt ' + ' '.join(f'v({node})' for node in terminal_nodes(devices)))
...
def read_buffer_trace(path: Path, expected_time):
    """Validate diagnostic buffer nodes against the main solver time axis."""
    data = np.loadtxt(path)
    if data.ndim != 2 or data.shape[1] != 8 or not np.isfinite(data).all():
        raise ValueError('malformed/nonfinite buffer transient data')
    if (not np.equal(data[:, 0::2], data[:, :1]).all()
            or not np.array_equal(data[:, 0], expected_time)):
        raise ValueError('buffer transient time axis differs from main trace')
    return data[:, 1::2]
