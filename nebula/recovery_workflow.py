...
exact deck and structural receiver export, drawing and design.json persistence. Receipt serialization
...
def _export(design, out):
    ...
    # Keep the exact selected CTLE bytes, including recorded newline identity.
    source = Path(design["physical_evidence"]["directory"]) / "design.cir"
    selected_bytes = source.read_bytes()
    if hashlib.sha256(selected_bytes).hexdigest() != design["physical_evidence"]["deck_sha256"]:
        raise ValueError("selected physical CTLE changed during export")
    receiver_files = []
    if _verified(design) and (design.get("request_match") or {}).get("request_met"):
        from nebula.generated_receiver import export as export_receiver
        metadata = export_receiver(design, selected_bytes.decode("ascii"), out / "receiver")
        metadata.update(artifact="receiver/receiver.cir", metadata_artifact="receiver/metadata.json")
        design["generated_receiver"] = metadata
        metadata_path = out / "receiver" / "metadata.json"
        metadata_path.write_text(json.dumps(metadata, indent=2, allow_nan=False) + "\n",
                                 encoding="utf-8", newline="\n")
        receiver_files = [out / "receiver" / "receiver.cir", metadata_path]
        from nebula.programmable_option import eligible, export_option
        if eligible(design, selected_bytes):
            try:
                option, option_files = export_option(design, selected_bytes, out / "receiver_programmable")
                design["programmable_receiver_option"] = option
                receiver_files.extend(option_files)
            except (OSError, ValueError, KeyError, TypeError) as exc:
                # This optional prototype cannot invalidate or relabel the fixed CTLE.
                design["programmable_receiver_option"] = dict(status="UNAVAILABLE", reason=str(exc), full_receiver_verified=False)
    written, warnings = write_outputs(
        design, out, deck=deck, extra_files={"explanation.txt": explanation})
    (out / "design.cir").write_bytes(selected_bytes)
    return list(written) + receiver_files, warnings
...
def run_request(text, out, *, selection_mode="rl", max_candidates=8, progress=None):
    ...
    try:
        ...
        if design is not None:
            ...
            if result.get("status") == "accepted":
                required.extend(out / name for name in
                                ("receiver/receiver.cir", "receiver/metadata.json"))
            if (design.get("programmable_receiver_option") or {}).get("receiver_deck_sha256"):
                required.extend(out / name for name in ("receiver_programmable/receiver.cir",
                                "receiver_programmable/metadata.json", "receiver_programmable/device_sheet.svg"))
    ...
    result["timing_scope"] = (
        ...
        "structural connected receiver export, drawing, explanation, design.json writes and required-output checks/hashes. "
    ...
    result["scope"] = ("Measured fixed physical CTLE in design.cir, with ideal behavioral DFE scoring outside that netlist; "
                       "receiver/receiver.cir is a separate structurally connected transistor CTLE+DFE export, "
                       "without nominal or full receiver verification. Where present, receiver_programmable is a separate saved nominal AC/signal prototype with signed-domain failures, not fresh verification or full receiver signoff. Full receiver/S7 signoff excluded.")
