...
def test_real_entry110_reproduces_without_promoting_scope():
    root = Path(__file__).resolve().parents[1]/'product_audits/entry110_dfe_buffer_memory_20260906'
    ...
    assert cfg['bits'] == bits().tolist() and cfg['buffers_connected']
    assert cfg['max_calls'] == saved['spice_calls'] == 1
    assert not cfg['comparator_connected'] and not cfg['ctle_connected']
    folder = root/'clean_tt_1.00_27'
    assert (folder/'design.cir').read_text() == M.deck(cfg['bits'], with_buffers=True)
    t,y = M.read_trace(folder/'trace.txt')
    z = D.read_buffer_trace(folder/'buffers.txt',t)
    r = M.analyze(t,y,cfg['bits'])
    assert all(v == saved[k] for k,v in r.items())
    assert r['memory_hold_pass'] and r['correct_bits'] == 32
    assert not r['hardware_dfe_complete'] and not r['full_receiver_verified']
    assert min(b['held_min_logic_margin_v'] for b in r['bits']) == pytest.approx(.8064779289468619)
    assert max(b['final_stable_delay_s'] for b in r['bits']) == pytest.approx(92.5e-12,abs=1e-15)
    for row in r['bits']:
        ps = (t-D.edge_time(row['bit_index']))*1e12
        active = 1 if row['expected_bit'] else 3
        low = np.flatnonzero((ps>=0)&(ps<=190)&(z[:,active]<.9))
        assert 37 <= ps[low[0]] <= 40
        assert ps[low[-1]] == pytest.approx(129.5,abs=1e-6)
        quiet = (ps>=110)&(ps<=190)&(z[:,1]>.9)&(z[:,3]>.9)
        assert quiet.sum() >= 60
    # Real undershoot is retained, not clipped to manufacture ideal logic.
    assert z[:,[1,3]].min() < 0
