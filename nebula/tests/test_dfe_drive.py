...
def test_real_entry108_reproduces_and_brief_high_is_not_a_pass():
    import json
    from nebula.experiments.exp_dfe_drive import mos_signature
    from nebula.experiments.exp_dfe_slicer import digest
    root = Path(__file__).resolve().parents[1] / 'product_audits/entry108_dfe_drive_20260906'
    ...
    assert summary['spice_calls'] == 4 and summary['selected_drive_um'] is None
    assert not summary['all_corner_block_pass'] and not summary['corners']
    for pair, expected_raw in zip(D.BUFFER_DRIVE_PAIRS_UM, (32, 32, 28, 28)):
        folder = root / f'screen_p{pair[0]}_n{pair[1]}_tt_1.00_27'
        ...
        assert all(value == saved[key] for key, value in result.items())
        assert sum(b['raw_min_logic_margin_v'] > 0 for b in result['bits']) == expected_raw
        assert sum(b['held_min_logic_margin_v'] > 0 for b in result['bits']) == 15
        assert not result['block_pass'] and result['stimulus_valid']
        assert mos_signature((folder / 'design.cir').read_text()) == saved['mos_signature']
        if pair == (8, 8):
            post = (t >= 1e-9) & (t <= 7.4e-9)
            assert y[post, 5].max() > .9  # A transient high does not satisfy the hold window.
