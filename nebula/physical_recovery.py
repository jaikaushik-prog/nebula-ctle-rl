...
MAX_CANDIDATES = 8
CONTINUOUS_RESULT = D.ROOT / 'nebula/product_audits/continuous_cs_coverage_20260915/base480_cs1p02/result.json'
CONTINUOUS_RESULT_SHA256 = 'ff41986b53f48970b80098d0680c92ec6a2b8c6a25cd1c727e3508c0fe6ca87c'
CONTINUOUS_MANIFEST_SHA256 = '26bd940e1f829f3020e3b1635ff0286df3094a3ea47210ba96917db8719f1b38'


def continuous_proposal(request):
    """Exact-target, hash-checked candidate selection only; never a live pass."""
    if request != {'peaking_db': 6., 'f_peak_hz': 2.5e9}:
        return None
    from nebula.device import continuous_cs_refinement as C
    folder = CONTINUOUS_RESULT.parent.resolve()
    manifest = folder / 'evidence_sha256.json'
    if (hashlib.sha256(CONTINUOUS_RESULT.read_bytes()).hexdigest() != CONTINUOUS_RESULT_SHA256
            or hashlib.sha256(manifest.read_bytes()).hexdigest() != CONTINUOUS_MANIFEST_SHA256):
        raise ValueError('continuous candidate accepted evidence changed')
    for name, expected in json.loads(manifest.read_text(encoding='utf-8')).items():
        ...
        if not path.is_relative_to(folder) or hashlib.sha256(path.read_bytes()).hexdigest() != expected:
            raise ValueError('continuous candidate raw evidence changed')
    previous = json.loads(CONTINUOUS_RESULT.read_text(encoding='utf-8'))
    search = C.make_candidate(1.02)
    if (not D.is_verified(previous) or not previous['request_match']['request_met']
            or previous['search']['u'] != search['u']
            or previous['search']['bank_setting'] is not None):
        raise ValueError('continuous candidate provenance does not match validated parameters')
    search['candidate_kind'] = 'continuous-cs-refinement'
    provenance = dict(prior_result=str(CONTINUOUS_RESULT.relative_to(D.ROOT)),
        prior_result_sha256=CONTINUOUS_RESULT_SHA256, prior_manifest_sha256=CONTINUOUS_MANIFEST_SHA256,
        base_result_sha256=C.BASE_SHA256, candidate_id=search['candidate_id'],
        base_setting=480, bank_setting=None, cs_factor=1.02, prior_pass_is_selection_only=True)
...
def prepare(request, selection_mode):
    ...
    continuous = continuous_proposal(request)
    if continuous is not None:
        proposals.insert(0, continuous)
        order.insert(0, continuous[0]['setting'])
    return proposals, {'eligible_settings': eligible, 'candidate_order': order,
        'continuous_candidates': [continuous[0]['candidate_id']] if continuous else [],
        'continuous_provenance': continuous[1] if continuous else None,
        ...
        try:
            with recovery_lock():
                ...
                for index, (search, legacy) in enumerate(proposals[:max_candidates]):
                    ...
                    try:
                        if search.get('candidate_kind') == 'continuous-cs-refinement':
                            from nebula.device import continuous_cs_refinement as C
                            if request != {'peaking_db': 6., 'f_peak_hz': 2.5e9} or search['candidate_id'] != 'base480_cs1p02':
                                raise ValueError('continuous dispatch identity/request mismatch')
                            attempt.update(candidate_kind='continuous-cs-refinement', base_setting=480,
                                           bank_setting=None, bank_code=None, cs_factor=1.02)
                            if progress:
                                progress('Fresh verification of continuous Cs refinement of base 480', 18)
                            # Same normal 137-call gate; its per-call timeouts remain unchanged.
                            design = C.run_candidate(1.02, folder, float('inf'))
                        else:
